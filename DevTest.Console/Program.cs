...
namespace DevTest.Console
{
	internal class Program
	{
		private static void Main(string[] args)
		{
			...
			words.ToList()
				.ForEach(
					word => System.Console.WriteLine("Word: {0}; IsPalindrome: {1}", word, word.IsPalindrome().ToString().ToLower()));
			...
			int counter = 0;
			foreach (var page in PageingHelper.Paginate(wordList, pageSize))
			{

				System.Console.WriteLine(page.Key);

				page.Value.ForEach(word => System.Console.WriteLine("{0}. {1}", ++counter , word));
			...
			var postAndPersons = PostProviderHelper.JointPostAndPersons(posts, persons).ToList();
			
			//implement exercise 3.1
			
			postAndPersons.ForEach(x =>
				System.Console.WriteLine("Person:{0} Content:{1}, Comment Count:{2}, Commenters:{3}", x.Person, x.PostContent,
					x.Commenter.Count, string.Join(", ", x.Commenter)));
			...
			postAndPersons.ForEach(x =>
				System.Console.WriteLine("{0}: {1}", x.PostContent, string.Join(", ", x.LikedBy)));
			...
			PostProviderHelper.GroupCommentsByCommenter(posts, persons, personsOlderThan25Predicate)
				...
				.ForEach(x =>
				{
					System.Console.WriteLine(x.Person);
					System.Console.WriteLine("-----");
					x.Comments.ForEach(System.Console.WriteLine);
					System.Console.WriteLine();
				});
